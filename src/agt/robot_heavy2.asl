 ...
 * Robot pesado 2: misma lógica que robot_heavy.
 * ============================================================================ */
...
-!select_shelf_and_execute(CId, Weight, W, H) : true <-
    .print("⚠️ [HEAVY2] Falló selección de estantería para ", CId);
...
+!execute_task(CId, ShelfId) : true <-
    ...
    warehouse.log_event("EVENT | time=",T_pk," | agent=robot_heavy2 | type=pickup | data=",CId);
...
-!execute_task(CId, ShelfId) : error(shelf_full, _) & carrying(CId) & expansion_count(CId, N) & N >= 2 <-
    ...
    warehouse.log_event("EVENT | time=",T_ex," | agent=robot_heavy2 | type=expansion_drop_final | data=",CId,",",ShelfId);
...
-!execute_task(CId, ShelfId) : error(shelf_full, _) & carrying(CId) <-
    ...
    -+carrying(none);
    .time(H_ex,M_ex,S_ex); T_ex=H_ex*3600+M_ex*60+S_ex;
    warehouse.log_event("EVENT | time=",T_ex," | agent=robot_heavy2 | type=expansion_drop | data=",CId,",",ShelfId);
    release_task(CId);
    !safe_return;
    unclaim_container(CId);
    !check_queue.
...
-!execute_task(CId, ShelfId) : not carrying(CId) <-
    release_task(CId);
    unclaim_container(CId);
    .send(supervisor, tell, task_failed(CId));
    .wait(2000);
    !safe_return;
    !check_queue.
...
-!execute_task(CId, ShelfId) : true <-
    ...
    warehouse.log_event("EVENT | time=",T_tf," | agent=robot_heavy2 | type=task_failed | data=",CId,",",ShelfId);
    .send(supervisor, tell, task_failed(CId));
    .wait(5000);
    !safe_return;
    !check_queue.
...
-!get_to_container(CId, 1) : true <-
    ...
    warehouse.log_event("EVENT | time=",T_nf," | agent=robot_heavy2 | type=nav_failed | data=",CId);
    .send(supervisor, tell, task_failed(CId));
    .fail.
...
// X<=14 going to right side (TX>=15): use x=19 to avoid both robots congesting x=9
...
+stored(CId, ShelfId) : true <-
    ...
    warehouse.log_event("EVENT | time=",T_st," | agent=robot_heavy2 | type=stored | data=",CId,",",ShelfId);
