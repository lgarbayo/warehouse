 ...
 * Robot medio: contenedores que no son del rango exclusivo del ligero
 * (Weight > 10 OR H > 1), hasta 30kg y 1x2.
 * ============================================================================ */

+container_at_entrance(CId, Type, Weight, W, H) : nav_failed(CId) <- true.

+container_at_entrance(CId, Type, Weight, W, H) :
    state(idle) & Weight <= 30 & W <= 1 & H <= 2 & Weight > 10 <-
    !try_claim(CId, Type, Weight, W, H).

+container_at_entrance(CId, Type, Weight, W, H) :
    state(idle) & Weight <= 30 & W <= 1 & H <= 2 & H > 1 <-
    !try_claim(CId, Type, Weight, W, H).
...
+!check_pending_containers :
    container_at_entrance(CId, Type, Weight, W, H) &
    Weight <= 30 & W <= 1 & H <= 2 & Weight > 10 &
    not nav_failed(CId) <-
    !try_claim(CId, Type, Weight, W, H).

+!check_pending_containers :
    container_at_entrance(CId, Type, Weight, W, H) &
    Weight <= 30 & W <= 1 & H <= 2 & H > 1 &
    not nav_failed(CId) <-
    !try_claim(CId, Type, Weight, W, H).
...
-!select_shelf_and_execute(CId, Weight, W, H) : true <-
    .print("⚠️ [MEDIUM] Falló selección de estantería para ", CId);
...
-!execute_task(CId, ShelfId) : true <-
    ...
    warehouse.log_event("EVENT | time=",T_tf," | agent=robot_medium | type=task_failed | data=",CId,",",ShelfId);
    .send(scheduler, tell, task_failed(CId));
    .wait(5000);
    !safe_return;
    !check_queue.
...
-!get_to_container(CId, 1) : true <-
    ...
    warehouse.log_event("EVENT | time=",T_nf," | agent=robot_medium | type=nav_failed | data=",CId);
    .send(scheduler, tell, task_failed(CId));
    .fail.
...
+stored(CId, ShelfId) : true <-
    ...
    warehouse.log_event("EVENT | time=",T_st," | agent=robot_medium | type=stored | data=",CId,",",ShelfId);
