...
+!execute_task(CId, ShelfId) : true <-
    ...
    .print("🚀 [HEAVY] Iniciando tarea: ", CId, " → ", ShelfId);
    ...
    .print("📦 [HEAVY] Fase 2: Recogiendo ", CId);
    -+state(picking);
    pickup(CId);
    ...
    .print("📥 [HEAVY] Fase 4: Depositando en ", ShelfId);
    -+state(dropping);
    drop_at(ShelfId);
    .wait(800);
...
+!navigate(TX, TY) : true <-
    .my_name(Me);
    .print("⚠️ [HEAVY] Nav timeout hacia (", TX, ",", TY, ")");
...
-!get_to_container(CId, 1) : true <-
    .print("⚠️ [HEAVY] Inaccesible tras 3 intentos: ", CId);
    -+carrying(none);
    release_task(CId);
    +nav_failed(CId);
...
+!execute_exit(CId, ShelfId) : true <-
    -nav_limit(_); +nav_limit(300);
    .print("🚀 [HEAVY] Execute exit: ", CId, " desde ", ShelfId);
...
-!execute_exit(CId, ShelfId) : exit_picked(CId) <-
    .print("⚠️ [HEAVY] Fallo en execute_exit tras pickup, devolviendo ", CId, " a ", ShelfId);
    -exit_claimed(CId);
    -exit_picked(CId);
    !return_to_shelf(CId, ShelfId);
    -+carrying(none);
...
-!execute_exit(CId, ShelfId) : true <-
    .print("⚠️ [HEAVY] Fallo en execute_exit para ", CId);
    -exit_claimed(CId);
    -+carrying(none);
    !check_queue.
