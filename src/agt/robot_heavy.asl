...
+outbound_available(CId, ShelfId, W, H, Weight) : state(idle) & max_weight(MaxW) & max_size(MaxDimW, MaxDimH)
    ...
    .print("📤 [HEAVY] Tomando outbound: ", CId, " de ", ShelfId);
...
+outbound_available(CId, ShelfId, W, H, Weight) : not state(idle) & max_weight(MaxW) & max_size(MaxDimW, MaxDimH)
                                                  & not (Weight > MaxW) & not (W > MaxDimW) & not (H > MaxDimH) <-
    .print("⚠️ [HEAVY] Ocupado, guardando outbound: ", CId).

+outbound_available(CId, ShelfId, W, H, Weight) : true <-
    -outbound_available(CId, ShelfId, W, H, Weight)[source(scheduler)].

+!execute_outbound_task(CId, ShelfId) : true <-
    .print("📤 [HEAVY] Fase 1: Navegando a estantería ", ShelfId);
    move_to_shelf(ShelfId);
    ?nav_target(TX, TY);
    !navigate(TX, TY);

    .print("📦 [HEAVY] Fase 2: Recogiendo de estantería ", ShelfId);
    -+state(picking);
    pickup_from_shelf(ShelfId, CId);
    .wait(1000);

    .print("🚚 [HEAVY] Fase 3: Navegando a zona outbound");
    -+state(carrying);
    move_to_outbound;
    ?nav_target(TX2, TY2);
    !navigate(TX2, TY2);

    .print("📤 [HEAVY] Fase 4: Entregando contenedor");
    -+state(dropping);
    drop_in_outbound(CId);
    .wait(1000);

    .print("✨ [HEAVY] Outbound completado: ", CId);
    -+carrying(none);
    release_task(CId);
...
-!execute_outbound_task(CId, ShelfId) : true <-
    .print("⚠️ [HEAVY] Fallo en tarea outbound: ", CId);
    -+carrying(none);
    release_task;
...
+!check_queue : outbound_available(CId, ShelfId, W, H, Weight) & max_weight(MaxW) & max_size(MaxDimW, MaxDimH)
              & not (Weight > MaxW) & not (W > MaxDimW) & not (H > MaxDimH) <-
    .print("✅ [HEAVY] Procesando outbound disponible: ", CId, " desde ", ShelfId);
    -outbound_available(CId, ShelfId, W, H, Weight)[source(scheduler)];
    accept_task(CId);
    -+state(working);
    -+carrying(CId);
...
-!check_queue : task(CId, ShelfId) <-
    .print("⚠️ [HEAVY] Fallo al navegar a base, procesando tarea encolada: ", CId);
    -task(CId, ShelfId)[source(scheduler)];
    accept_task(CId);
    -+state(working);
    -+carrying(CId);
...
+state(idle) : task(CId, ShelfId) <-
    .print("✅ [HEAVY] Tarea pendiente al quedar idle: ", CId, " a ", ShelfId);
    -task(CId, ShelfId)[source(scheduler)];
    accept_task(CId);
    -+state(working);
    -+carrying(CId);
